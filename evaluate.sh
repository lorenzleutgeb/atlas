...
function run {
	now
	(set -x ; atlas run --infer --equal-ranks=true $ARGS $@)
	now
}
...
run --simple-annotations=true RandTree.descend
...
run --simple-annotations=true CoinSearchTree.insert CoinSearchTree.delete

say '\n### Randomised Meldable Heap'
# NOTE: insert/delete_min calls meld
run --simple-annotations=true RandMeldableHeap.insert RandMeldableHeap.delete_min
...
do
	...
	do
		say "\n### p = ${P}, c = ${C}\n"
		run --simple-annotations=true RandSplayTreeMatrix.P${P//\//}C${C//\//}.splay
	done
done
...
run --simple-annotations=true SplayTree.insert SplayTree.delete
...
say '\n## Pairing Heap'
run --simple-annotations=true PairingHeap.insert_isolated PairingHeap.merge_pairs_isolated PairingHeap.delete_min_via_merge_pairs_isolated PairingHeap.merge_isolated
